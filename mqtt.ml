...
let get_remaining_len instream = 
  ...
  let value = 0 
  do_while (fun _ ->
              let buffer_size = 1 in 
              let buffer = String.create buffer_size in 
              let bytes_back = read instream buffer 0 1 in (* get the next byte *)
              let digit = Char.code buffer.[0]
              value += (digit land 127) * multiplier 
              multiplier *= 128; digit) 
            (fun digit -> ((digit AND 128) != 0))
*)
